...
surveys.post('/', async (c) => {
  ...
  const baseSlug = title.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)+/g, '') || 'survey'
  const slug = `${baseSlug}-${id.slice(0, 6)}`

  try {
    await c.env.DB.prepare(
      `INSERT INTO surveys (id, user_id, title, slug) VALUES (?, ?, ?, ?)`
    )
...
surveys.put('/:id', async (c) => {
  ...
  try {
    ...
    const targetSurvey = await db.prepare(
    ...
    statements.push(
      db.prepare(
        `UPDATE surveys SET title = ?, primary_color = ?, logo_url = ? WHERE id = ?`
